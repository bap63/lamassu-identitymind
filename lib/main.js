...
function verifyUser (rawData, cb) {
  ...
  if (data.pm) { return stage1Verify(data, cb) }
...
function _normalizeData (rawData) {
  ...
  var man = (manPrefix + license.uid).substr(0, 60)
  ...
  if (rawData.transaction) {
    ...
    if (transaction.toAddress) data.pbc = bitcoinAddressHash(transaction.toAddress)
  }
